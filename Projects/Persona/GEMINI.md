...
```markdown
@./config/env.md
@./instances/MHM/agents/researcher.md
```

### Context Advantage
Gemini has 1M token context window - ideal for:
- Large codebase analysis
- Extensive research tasks
- Multi-file context understanding
- Researcher agent with broad scope queries

## Model Configuration

Gemini CLI uses **gemini-2.0-flash** (no model selection).

To use Gemini for an agent, add to agent frontmatter:
```yaml
provider: gemini
```
...
# Test an agent with Gemini
./scripts/run-agent.sh MHM researcher process-research-queue 60
# Note: Requires provider: gemini in agent frontmatter
...
When run-agent.sh invokes Gemini:
```bash
echo "$prompt" | gemini --sandbox false
```

The `--sandbox false` flag allows file system access for agent operations.
